...
(function () {
  ...
  const onScroll = () => {
    ...
    if (nav) {
      ...
      if (nav.hasAttribute('data-nav-fixed')) { nav.classList.add('scrolled'); return; }
  ...
  const sections = [
    ...
    { id: 'servicios', num: '02', label: 'Servicios' },
    { id: 'productos', num: '03', label: 'Productos' },
    { id: 'galeria',   num: '04', label: 'Estilo' },
  ...
  // ============ PREMIUM TEXT SPLIT (titles by word) ============
  document.querySelectorAll('.section-title').forEach((el) => {
    if (el.dataset.split === 'no' || el.querySelector('.word')) return;
    ...
    words.forEach((w, i) => {
      ...
      inner.textContent = w;
      inner.style.setProperty('--d', `${i * 0.06}s`);
  ...
  const revealTargets = document.querySelectorAll(
    ...
    '.cta-banner-content, .stat, section'
  );
  revealTargets.forEach(el => {
    if (el.tagName !== 'SECTION') el.classList.add('reveal');
  });

  const titlesToAnimate = document.querySelectorAll('.section-title.split-title');
  ...
  if ('IntersectionObserver' in window) {
    ...
    titlesToAnimate.forEach(el => io.observe(el));
  } else {
    revealTargets.forEach(el => el.classList.add('in'));
    titlesToAnimate.forEach(el => el.classList.add('in'));
  }
